...
--* UPGRADE "1" --> "2"
 ...
 EXPORT CSV
	WITH HEADER
	FILE "output/export11.csv" ENCODING "UTF-8"
	FROM
SELECT * FROM TEMP1;

EXPORT CSV
	FILE "output/export12.csv" ENCODING "UTF-8" FROM
SELECT ID, PICTURE, TEXT, TEXT2
FROM TEMP1;

 EXPORT CSV
	DATE AS TIMESTAMP
	FILE "output/export13.csv" ENCODING "UTF-8"
FROM SELECT * FROM TEMP1;

EXPORT CSV
FILE "output/folder/export14.csv" ENCODING "UTF-8"
COLUMN DATE1 SKIP
FROM
